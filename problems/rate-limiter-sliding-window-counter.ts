...
Deno.test("Sliding Window Counter Rate Limiter - basic functionality", () => {
  ...
  // Fill first window
  for (let i = 0; i < 8; i++) {
    assert(limiter.allowRequest("user1", 100 + i * 100) === true);
  }
  ...
  assert(limiter.allowRequest("user1", 1201) === true);
  assert(limiter.allowRequest("user1", 1202) === false); // Should exceed weighted limit
});
...
Deno.test("Sliding Window Counter Rate Limiter - window transition", () => {
  ...
  for (let i = 0; i < 5; i++) {
    assert(limiter.allowRequest("user1", 200 + i * 100) === true);
  }
  assert(limiter.allowRequest("user1", 700) === false);
  
  // Right at window boundary - previous window weight is 0
  assert(limiter.allowRequest("user1", 1200) === true); // New window, count resets
});
...
Deno.test("Sliding Window Counter Rate Limiter - weighted calculation accuracy", () => {
  ...
  assert(limiter.allowRequest("user1", 1501) === true);
  
  // Now have 2 in current window
  // Previous: 8 * 0.5 = 4
  // Current: 2
  // Total: 6
  assert(limiter.allowRequest("user1", 1502) === true);
  assert(limiter.allowRequest("user1", 1503) === true);
  assert(limiter.allowRequest("user1", 1504) === true);
  assert(limiter.allowRequest("user1", 1505) === true);
  
  // Total should be around 10, next should fail
  assert(limiter.allowRequest("user1", 1506) === false);
});
...
Deno.test("Sliding Window Counter Rate Limiter - multiple users", () => {
  ...
  assert(limiter.allowRequest("user1", 300) === false);
  
  // User 2 (independent)
  assert(limiter.allowRequest("user2", 50) === true);
  assert(limiter.allowRequest("user2", 150) === true);
  assert(limiter.allowRequest("user2", 250) === true);
  assert(limiter.allowRequest("user2", 350) === false);
});
...
Deno.test("Sliding Window Counter Rate Limiter - long time gaps", () => {
  ...
  assert(limiter.allowRequest("user1", 5000) === true);
  assert(limiter.allowRequest("user1", 5100) === true);
  assert(limiter.allowRequest("user1", 5200) === true);
  assert(limiter.allowRequest("user1", 5300) === true);
  assert(limiter.allowRequest("user1", 5400) === true);
  assert(limiter.allowRequest("user1", 5500) === false);
});