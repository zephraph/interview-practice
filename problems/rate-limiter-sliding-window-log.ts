...
Deno.test("Sliding Window Log Rate Limiter - basic functionality", () => {
  ...
  assert(limiter.allowRequest("user1", 300) === true);
  assert(limiter.allowRequest("user1", 600) === true);
  assert(limiter.allowRequest("user1", 900) === false); // 4th request within 1000ms
  
  // After some requests expire from window
  assert(limiter.allowRequest("user1", 1100) === true); // First request (at 0) is now outside window
  assert(limiter.allowRequest("user1", 1200) === false); // Still have 3 requests in window
  assert(limiter.allowRequest("user1", 1400) === true); // Request at 300 is now outside window
});
...
Deno.test("Sliding Window Log Rate Limiter - exact window boundaries", () => {
  ...
  assert(limiter.allowRequest("user1", 0) === true);
  assert(limiter.allowRequest("user1", 500) === true);
  assert(limiter.allowRequest("user1", 999) === false); // All requests still in window
  assert(limiter.allowRequest("user1", 1000) === true); // First request exactly out of window
  assert(limiter.allowRequest("user1", 1499) === false); // Request at 500 still in window
  assert(limiter.allowRequest("user1", 1500) === true); // Request at 500 now out of window
});
...
Deno.test("Sliding Window Log Rate Limiter - memory cleanup", () => {
  ...
  // Add many requests
  for (let i = 0; i < 3; i++) {
    assert(limiter.allowRequest("user1", i * 100) === true);
  }
  
  // Jump far ahead - old timestamps should be cleaned up
  assert(limiter.allowRequest("user1", 10000) === true);
  assert(limiter.allowRequest("user1", 10100) === true);
  assert(limiter.allowRequest("user1", 10200) === true);
  assert(limiter.allowRequest("user1", 10300) === false);
});
...
Deno.test("Sliding Window Log Rate Limiter - burst handling", () => {
  ...
  // Burst of requests
  for (let i = 0; i < 5; i++) {
    assert(limiter.allowRequest("user1", 1000 + i) === true);
  }
  assert(limiter.allowRequest("user1", 1005) === false);
  
  // Still rate limited throughout the window
  assert(limiter.allowRequest("user1", 3000) === false);
  assert(limiter.allowRequest("user1", 5999) === false);
